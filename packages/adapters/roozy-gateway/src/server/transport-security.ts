export const INSECURE_REMOTE_HTTP_ESCAPE_HATCH = "allowInsecureHttp";
...
export function allowsInsecureRemoteHttp(
  config: Record<string, unknown>,
): boolean {
  return (
    parseBooleanLike(config[INSECURE_REMOTE_HTTP_ESCAPE_HATCH]) === true
  );
}

export function remotePlainHttpDeniedMessage(hostname: string): string {
  return (
    `Roozy AI Gateway baseUrl uses remote plain HTTP for "${hostname}". ` +
